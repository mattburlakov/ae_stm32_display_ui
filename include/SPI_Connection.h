...
class SPI_Connection {
     ...
     SPI_Connection() {
          ...
          GPIOB->MODER &=
              ~(GPIO_MODER_MODE12 | GPIO_MODER_MODE13 | GPIO_MODER_MODE14 |
                GPIO_MODER_MODE15);  // PB13, PB15 - альтернативная функция,
                                     // PB12, PB14 - выход
          GPIOB->MODER |=
              (GPIO_MODER_MODE12_0 | GPIO_MODER_MODE13_1 | GPIO_MODER_MODE14_0 |
               GPIO_MODER_MODE15_1);  // PB13, PB15 - альтернативная функция,
                                      // PB12, PB14 - выход
          GPIOB->AFR[1] &=
              ~(GPIO_AFRH_AFSEL13 | GPIO_AFRH_AFSEL15);  // AF5 для PB13, PB15
          GPIOB->AFR[1] |=
              (GPIO_AFRH_AFSEL13_2 | GPIO_AFRH_AFSEL13_0 | GPIO_AFRH_AFSEL15_2 |
               GPIO_AFRH_AFSEL15_0);  // AF5 для PB13, PB15

          GPIOB->BSRR |= GPIO_BSRR_BR14;  // шлем сброс на PB14

          RCC->APB1ENR1 |=
              RCC_APB1ENR1_SPI2EN;  // включаем SPI2 на тактирование

          SPI2->CR1 |= SPI_CR1_MSTR | SPI_CR1_CPOL | SPI_CR1_CPHA |
                       SPI_CR1_BR_0 | SPI_CR1_BR_1 | SPI_CR1_SSM |
                       SPI_CR1_SSI;  // включаем режим мастера, полярность
                                     // тактового сигнала
     ...
     void SSD1322_write_C(uint8_t cmd) { write_data(cmd); }
     ...
     void SSD1322_write_C4D(uint8_t cmd,
                            uint8_t D0,
                            uint8_t D1,
                            uint8_t D2,
                            uint8_t D3) {
     ...
     void SSD1322_send_graphics(uint8_t* buf, uint16_t size) {
          SSD1322_write_C2D(0x15, 28,
                            91);  // set column address, start, end
          SSD1322_write_C2D(
              0x75, 0,
              63);  // set row address, moved out of the loop (issue 302)
