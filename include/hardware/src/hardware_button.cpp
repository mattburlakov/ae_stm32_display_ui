#include "hardware_button.h"

#include "hardware_gpio.h"

#include <stdint.h>

struct ButtonConfig {
     enum GpioPin pin_common;
     enum GpioPin pin_button;
};

static const struct ButtonConfig buttons[] = {
     [ HARDWARE_BUTTON_1 ] = {
          .pin_common = GPIO_PIN_COMMON_4,
          .pin_button = GPIO_PIN_BUTTON_READ_1,
     },
     [ HARDWARE_BUTTON_2 ] = {
          .pin_common = GPIO_PIN_COMMON_4,
          .pin_button = GPIO_PIN_BUTTON_READ_2,
     },
     [ HARDWARE_BUTTON_3 ] = {
          .pin_common = GPIO_PIN_COMMON_4,
          .pin_button = GPIO_PIN_BUTTON_READ_3,
     },
     [ HARDWARE_BUTTON_4 ] = {
          .pin_common = GPIO_PIN_COMMON_4,
          .pin_button = GPIO_PIN_BUTTON_READ_4,
     },
     [ HARDWARE_BUTTON_5 ] = {
          .pin_common = GPIO_PIN_COMMON_3,
          .pin_button = GPIO_PIN_BUTTON_READ_4,
     },
     [ HARDWARE_BUTTON_TIMER ] = {
          .pin_common = GPIO_PIN_COMMON_3,
          .pin_button = GPIO_PIN_BUTTON_READ_3,
     },
     [ HARDWARE_BUTTON_SET ] = {
          .pin_common = GPIO_PIN_COMMON_3,
          .pin_button = GPIO_PIN_BUTTON_READ_2,
     },
     [ HARDWARE_BUTTON_MENU ] = {
          .pin_common = GPIO_PIN_COMMON_3,
          .pin_button = GPIO_PIN_BUTTON_READ_1,
     },
     [ HARDWARE_BUTTON_ON ] = {
          .pin_common = GPIO_PIN_COMMON_1,
          .pin_button = GPIO_PIN_BUTTON_READ_3,
     },
     [ HARDWARE_BUTTON_OFF ] = {
          .pin_common = GPIO_PIN_COMMON_1,
          .pin_button = GPIO_PIN_BUTTON_READ_4,
     },
};

static uint16_t bitmask = 0x0000;

static uint16_t pin_state( uint8_t idx ) {
     return ( hardware_gpio_pin_get( buttons[ idx ].pin_button ) == 0 ) ? 1 : 0;
}

bool hardware_button_check_state( enum Button button ) {
     return bitmask & ( 0x0001 << button );
}

void hardware_button_update( void ) {
     bitmask = 0x0000;
     for( uint8_t idx = 0; idx < HARDWARE_BUTTON_COUNT; ++idx ) {
          hardware_gpio_pin_set( buttons[ idx ].pin_common );
          bitmask |= ( uint16_t )( pin_state( idx ) << idx );
          hardware_gpio_pin_reset( buttons[ idx ].pin_common );
     }
}
